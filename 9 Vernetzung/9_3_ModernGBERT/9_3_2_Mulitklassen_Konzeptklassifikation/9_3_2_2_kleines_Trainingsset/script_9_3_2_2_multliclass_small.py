...
# Special feature: the test data set is larger than the training data set
# (one fold is used for training, rest for testing).
# ================================================================
...
# ================================================================
# Concept and term extraction
# ---------------------------------------------------------------
# Traverses hierarchical JSON vocab structures and builds a mapping
# from concept paths (keys) to their associated term lists.
# ================================================================
def extract_concepts_and_terms(
    json_nodes: Dict[str, Any],
...
# Concept key lookup
# ---------------------------------------------------------------
# Resolves a concept label to its corresponding key in the mapping.
...
def find_concept_key(label: str, mapping: Dict[str, List[str]]) -> str:
    ...
    if label_stripped in mapping:
        ...
        raise ValueError(
            f"Concept '{label}' not found in mapping. "
...
# ================================================================
# Hybrid-stratified fold assignment
# ---------------------------------------------------------------
# Combines stratified sampling for large classes with random
# distribution for underrepresented concepts.
# ================================================================
def assign_hybrid_folds(samples, folds=10, seed=42):
    ...
    # Small classes: random assignment
    ...
    assert (fold_ids >= 0).all(), "Not all samples received a fold assignment!"
    return fold_ids
...
# ---------------------------------------------------------------
# Performs k-fold training and evaluation of a SentenceTransformer
# model to map lemma–definition pairs to concept labels.
# ================================================================
def run_cv(
    tsv_path: str,
    vocab_json_path: str,
    ...
    if "Lemma_bereinigt" not in df.columns:
        raise ValueError("TSV must also contain column 'Lemma_bereinigt'.")
    ...
    for fold_idx in range(folds):
        ...
        for s in train_samples:
            text_a = f"{s['lemma']} — {s['definition']}" if s["lemma"] else s["definition"]
            train_examples.append(InputExample(texts=[text_a, " ; ".join(s["terms"])]))
        ...
        # -------------------------
        eval_texts = [
            f"{s['lemma']} — {s['definition']}" if s["lemma"] else s["definition"]
...
fold_metrics, summary = run_cv(
    ...
    vocab_json_path="input/trinken_vokabular.json",
