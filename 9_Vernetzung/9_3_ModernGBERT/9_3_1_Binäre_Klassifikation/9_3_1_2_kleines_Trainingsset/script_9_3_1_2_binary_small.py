...
for fold, (train_idx, test_idx) in enumerate(skf.split(texts, labels), 1):
    ...
    print(f"Train size: {len(test_idx)}, Test size: {len(train_idx)}")
    fold_start_time = time.time()
    
    # ------------------------------------------------------------
    # Step 1: Train / Test Split
    # Conventional logic reversed: 1 fold for training, the rest for testing
    # ------------------------------------------------------------
    train_texts = [texts[i] for i in test_idx]      
    train_labels = [labels[i] for i in test_idx]
    test_texts = [texts[i] for i in train_idx]      
    test_labels = [labels[i] for i in train_idx]
